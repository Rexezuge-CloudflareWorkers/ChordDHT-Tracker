...
export default tseslint.config(
  ...
  unicorn.configs['flat/recommended'],
  {
    rules: {
      ...
      // forEach is used intentionally throughout the codebase in utility layers
      'unicorn/no-for-each': 'off',
      // .then() chains in DAO and utility layers are often intentional — warn, don't block
  ...
  // Layer 0: shared — zero @chord-dht-tracker/* deps
  {
    files: ['packages/shared/**/*.{ts,js}'],
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['@chord-dht-tracker/*'], message: 'shared must not import from other @chord-dht-tracker packages — it is a zero-dependency base layer' },
        ],
      }],
    },
  },
  // Layer 0: backend-errors — zero @chord-dht-tracker/* deps
  {
    files: ['packages/backend-errors/**/*.{ts,js}'],
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['@chord-dht-tracker/*'], message: 'backend-errors must not import from other @chord-dht-tracker packages — it is a zero-dependency base layer' },
        ],
      }],
    },
  },
  // Layer 1: backend-runtime — only shared and backend-errors
  {
    ...
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['@chord-dht-tracker/backend-data', '@chord-dht-tracker/backend-data/*'], message: 'backend-runtime must not import from backend-data (higher layer)' },
          { group: ['@chord-dht-tracker/backend-services', '@chord-dht-tracker/backend-services/*'], message: 'backend-runtime must not import from backend-services (higher layer)' },
          { group: ['@chord-dht-tracker/api', '@chord-dht-tracker/api/*'], message: 'backend-runtime must not import from apps/api' },
          { group: ['@chord-dht-tracker/background', '@chord-dht-tracker/background/*'], message: 'backend-runtime must not import from apps/background' },
        ],
      }],
    },
  },
  // Layer 2: backend-data — only shared and backend-errors
  {
    ...
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['@chord-dht-tracker/backend-runtime', '@chord-dht-tracker/backend-runtime/*'], message: 'backend-data must not import from backend-runtime' },
          { group: ['@chord-dht-tracker/backend-services', '@chord-dht-tracker/backend-services/*'], message: 'backend-data must not import services (higher layer)' },
          { group: ['@chord-dht-tracker/api', '@chord-dht-tracker/api/*'], message: 'backend-data must not import from apps/api' },
          { group: ['@chord-dht-tracker/background', '@chord-dht-tracker/background/*'], message: 'backend-data must not import from apps/background' },
        ],
      }],
    },
  },
  // Layer 3: backend-services — cannot import apps
  {
    ...
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['@chord-dht-tracker/api', '@chord-dht-tracker/api/*'], message: 'backend-services must not import from apps/api' },
          { group: ['@chord-dht-tracker/background', '@chord-dht-tracker/background/*'], message: 'backend-services must not import from apps/background' },
        ],
      }],
    },
  },
  // Layer 5: apps/api — route through backend-services, not directly to DAOs
  {
    files: ['apps/api/**/*.{ts,js}'],
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['@chord-dht-tracker/backend-data/dao', '@chord-dht-tracker/backend-data/dao/*'], message: 'apps/api must not import DAOs directly; use @chord-dht-tracker/backend-services instead (type-only imports are allowed)', allowTypeImports: true },
        ],
      }],
    },
  },
