...
- Integration: `pnpm run test:integration` (`vitest run --config test/integration/vitest.config.mts`) — CI `integration-tests` job. Uses `@cloudflare/vitest-pool-workers` (no V8 coverage — no thresholds there, omitted intentionally). 21 files in `api/` + `dao/` sharing `helpers/setup.ts` (migrations + secrets-store keys + `SELF` request helpers + real-crypto factories; cert `node_id` values must equal `uriNodeId(uri)`) — see `helpers/migrations.ts:splitSql`. `wrangler.test.jsonc` pins 7 `STABLE_BASE_MEMBERS` (`https://stable-0..6.example`, min-size 6) so member liveness is exercisable.
...
**Covered** (test files exist in `test/`): all 15 tracker endpoints (`health`, `stats`, `nodes`, `nodeGetDelete`, `seeds`, `heartbeat`, `crl`, `regions`, `geo`, `policy`, `stable_base`, `adminVerify`) plus route mapping (`ibaseRoute`, `nodesPostMapping`), services (`nodeService`, `vnodeService`, `maintenanceService`, `authService`, `certService`, `crlService`, `stableBaseService`, `statsService`, `defaultDeps`), DAOs (`nodeDao`, `vnodeDao`, `crlDao`, `trackerMetaDao`, `baseDao`, `cryptoService`, `d1Utils`), shared/data utils (`utils/sharedUtils`, `utils/dataUtils`), DI (`container`, `requestScope`, `serviceContext`, `errorTaxonomy`), cron/stale-cleanup (`cron/staleCleanup`, `workers/staleCleanupWorker`, `workers/scheduledForwarding`). Integration (`test/integration/api/`, `test/integration/dao/`): the 15 endpoint files above plus `stable-base-members` (live/stale/degraded/emergency + threshold math), `vnode-policy-enforcement` (inline cap, standalone register, bad proof, vnode delete), `crl-enforcement` (supersede lifecycle + documented revocation-enforcement gap), `d1-session-bookmark` (`x-d1-bookmark` expose + round-trip), `error-envelope` (BadRequest/NotFound/Unauthorized/Conflict sweep), `dao/scheduled-forwarding` (DO prune, 24h boundary, idempotency). Mocks in `test/mocks/` (`d1.ts` incl. `withSession`/`getBookmark`, `env.ts`, `cloudflare-workers.ts`); cloudflare socket/worker/workflow aliases are wired in `vitest.config.mts`.
